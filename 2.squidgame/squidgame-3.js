...
async function showQuestionMark(x, y, ticks) {
    if ( ticks === 0 )
        return
    let sQM = new Sprite(x, y)
    sQM.collider = 'static'
    sQM.textColor = '#2996d8'
    sQM.text = "?"
    sQM.layer = 60
    sQM.fill = sQM.stroke = color(0, 0, 0, 0)
    const startTick = cgt.getTick()
    return new Promise((resolve, reject) => {
        sQM.update = function() {
            let animPercent = (cgt.getTick() - startTick) / ticks
            if ( animPercent > 1.0 ) {
                sQM.remove()
                resolve()
                return
            }
            sQM.textSize = 70 + animPercent * 30
            sQM.opacity = Math.sin(animPercent * Math.PI)
        }
    })
}

async function showOutsidePlayersQuestionMarks() {
    let promises = []
    for ( const sP of sOutsidePlayers )
        promises.push(showQuestionMark(sP.x, sP.y - 70, 1200))
    await Promise.all(promises)
}
...
function showResult(roomId, result) {
    let middle = (roomId !== null ? roomMiddlePoss[roomId] : outsideMiddlePos)
...
async function goToRoom(roomId) {
    recGoToRoom(roomId)
    let sPlayer = rmRndArrayElem(sOutsidePlayers)
    sRoomPlayers[roomId].push(sPlayer)
    let roomPos = roomMiddlePoss[roomId]
    for ( const rp of roomPoss[roomId] ) {
        let playerOnPos = false
        for ( const sRP of sRoomPlayers[roomId] )
            if ( Math.sqrt((sRP.x - rp.x) ** 2 + (sRP.y - rp.y) ** 2) < 0.01 ) {
                playerOnPos = true
                break
            }
        if ( !playerOnPos ) {
    ...
    await cgt.moveSprite(sPlayer, roomPos, {speedPxPerTickS: 300})
}
...
cmdRecorder.addCommand('goToRoom', recGoToRoom, goToRoom)
...
for ( const level of levels ) {
    ...
    let pgtrProm = playersGoToRooms(level)
    await cgt.tickSleep(1100)
    let sopqmProm = showOutsidePlayersQuestionMarks()
    await Promise.all([pgtrProm, sopqmProm])

    await cgt.tickSleep(250)
